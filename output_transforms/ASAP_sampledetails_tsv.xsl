...
<xsl:value-of select="@name"/><xsl:text>&#x9;</xsl:text><xsl:value-of select="amplicon/@reads"/><xsl:text>&#x9;</xsl:text><xsl:value-of select='format-number(amplicon/breadth, "##.##")'/>%<xsl:text>&#x9;</xsl:text><xsl:value-of select="amplicon/significance"/><xsl:text>&#xa;</xsl:text>
</xsl:for-each> 
<xsl:text/>
</exsl:document>
