...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:exsl="http://exslt.org/common" xmlns:str="http://exslt.org/strings" xmlns:re="http://exslt.org/regular-expressions" extension-element-prefixes="exsl str re">
    ...
    <xsl:template match="/analysis">
	    ...
	    <br/>
	    	...
	    	<xsl:for-each select="sample[1]">
	    	    <xsl:for-each select="assay[starts-with(@name, 'Bp_') or starts-with(@name, 'IPSC')]">
	    	...
	    	<td class="norotate"><em>Bp</em> Positives</td>
	    	<td class="norotate"><em>Bp/Bm</em> Positives</td>
	    	<td class="norotate"><em>Bm</em> Positives</td>
	    	<td class="norotate">Near Neighbor</td>
	    	<td class="norotate" colspan="5" align="center"><em>B. pseudomallei</em></td>
	    	<td class="norotate" colspan="3" align="center"><em>B. pseudomallei/mallei</em></td>
	    	<td class="norotate" colspan="2" align="center"><em>B. mallei</em></td>
	    	<td class="norotate" colspan="3" align="center"><em>B. pseudomallei</em> complex</td>
	    	<td class="norotate" align="center"><em>Cepacia</em> complex</td>
	    	<td class="norotate" colspan="4" align="center">AMR assays</td>
	    	<!--<td class="norotate" colspan="5" align="center">AMR assays</td>-->
	    	...
	    	<xsl:variable name="count_bpbmnn">
	    	    ...
	    	    <xsl:for-each select="assay[starts-with(@name, 'Bp_') or starts-with(@name, 'IPSC')]">
                        ...
                        <xsl:variable name="CELL_COLOR">
                            <xsl:choose><xsl:when test="$ALIGNED_READS div $TOTAL_READS &gt;= 0.05">30.0</xsl:when>
                            <xsl:otherwise><xsl:value-of select="($ALIGNED_READS div $TOTAL_READS) * -1400 + 100"/></xsl:otherwise></xsl:choose>
                        </xsl:variable>
			...
			<xsl:when test="@type = 'SNP'">
                            ...
                            <xsl:when test="amplicon/@reads &gt;= $DEPTH_FILTER and amplicon//snp[@name != 'unknown']/snp_call/@percent &gt;= $PROP_FILTER">
                                <xsl:choose>
                                <xsl:when test="position() &gt;= 1 and position() &lt;= 5">Bp</xsl:when>
                                <xsl:when test="position() &gt;= 6 and position() &lt;= 8">Bp/Bm</xsl:when>
                                <xsl:when test="position() &gt;= 9 and position() &lt;= 10">Bm</xsl:when>
                                <xsl:when test="position() &gt;= 11 and position() &lt;= 13">Bpc</xsl:when>
                                <xsl:when test="position() = 14">NN</xsl:when>
	...
	<exsl:document method="html" href="{/analysis/@run_name}/Bp_{@name}.html">
	    <html>
	    <head>
	    ...
	    <body>
	    	...
	    	<br />
	    	<xsl:variable name="count_bp">
                    <xsl:value-of select="count(./assay/amplicon[contains(significance, 'Burkholderia pseudomallei') and not(contains(significance, ' mallei'))]//significance[not(@flag)]) + count(./assay/amplicon/snp[contains(significance, 'Burkholderia pseudomallei') and not(contains(significance, ' mallei')) and not(contains(significance, 'complex'))]//significance[not(@flag)])"/>
                </xsl:variable>
	    	<xsl:variable name="count_bpbm">
                    <xsl:value-of select="count(./assay/amplicon[contains(significance, 'Burkholderia pseudomallei or mallei')]//significance[not(@flag)]) + count(./assay/amplicon/snp[contains(significance, 'Burkholderia pseudomallei')]//significance[not(@flag)])"/>
                </xsl:variable>
	    	<xsl:variable name="count_bm">
                    <xsl:value-of select="count(./assay/amplicon[contains(significance, 'Burkholderia mallei')]//significance[not(@flag)]) + count(./assay/amplicon/snp[contains(significance, 'Burkholderia mallei')]//significance[not(@flag)])"/>
                </xsl:variable>
