...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:asap="http://pathogen.tgen.org/ASAP/functions" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:exsl="http://exslt.org/common" xmlns:str="http://exslt.org/strings" xmlns:re="http://exslt.org/regular-expressions" extension-element-prefixes="exsl str re asap">
    ...
    <xsl:template match="/analysis">
        <html>
        <head>
            <title>BT-Seq Run Summary for: <xsl:value-of select="@run_name"/></title>
            <style type="text/css">
	    ...
	    <br/>
	    	    ...
	    	    <xsl:for-each select="assay">
                    ...
                    <xsl:choose>
                        <xsl:when test="@type = 'SNP'">
                            <xsl:for-each select="asap:distinct-values(amplicon/snp[@name != 'unknown']/@name)">
                                <th class="rotate"><div><span>SNP - <xsl:value-of select='$CURRENT_ASSAY'/><xsl:if test="//assay[@name=$CURRENT_ASSAY]/@gene != ''"> - <xsl:value-of select='.'/></xsl:if></span></div></th>
                            </xsl:for-each>
                        </xsl:when>
	    	        <xsl:otherwise><th class="rotate"><div><span><xsl:value-of select='@name'/></span></div></th></xsl:otherwise>
                    </xsl:choose>
	    	    </xsl:for-each>
	    	</xsl:for-each>
	    	</tr>
		    ...
		    <xsl:for-each select="assay">
                        <xsl:variable name="CURRENT_ASSAY"><xsl:copy-of select="./*"/></xsl:variable>
                        <xsl:variable name="ALIGNED_READS"><xsl:value-of select="sum(.//amplicon/@reads)"/></xsl:variable>
                        <xsl:variable name="CELL_COLOR">
                            <xsl:choose><xsl:when test="sum(.//amplicon/@reads) div $TOTAL_READS &gt;= 0.05">30.0</xsl:when>
                            <xsl:otherwise><xsl:value-of select="(sum(.//amplicon/@reads) div $TOTAL_READS) * -1400 + 100"/></xsl:otherwise></xsl:choose>
                        </xsl:variable>
                        <xsl:choose>
			<xsl:when test="@type = 'SNP'">
                            <xsl:for-each select="asap:distinct-values(./amplicon//snp[@name != 'unknown']/@name)">
                                <xsl:choose>
                                <xsl:when test="$ALIGNED_READS &gt;= $MUT_DEPTH">
                                    <td class="norotate" align="center" style="background: hsl(0, 100%, {$CELL_COLOR}%)">
                                        <xsl:choose>
                                        <xsl:when test="$ALIGNED_READS &gt;= $DEPTH_FILTER and exsl:node-set($CURRENT_ASSAY)/amplicon//snp[@name=.]/snp_call/@count &gt;= $MUT_DEPTH and exsl:node-set($CURRENT_ASSAY)/amplicon//snp[@name=.]/snp_call/@percent &gt;= $PROP_FILTER">+</xsl:when>
                                        <xsl:otherwise>-</xsl:otherwise>
                                        </xsl:choose>
                                    </td>
                                </xsl:when>
                                <xsl:otherwise>
                                    <td class="norotate" align="center">-</td>
                                </xsl:otherwise>
                                </xsl:choose>
                            </xsl:for-each>
			</xsl:when>
                        <xsl:when test="contains(@name, 'pbp3')">
	    	            <xsl:variable name="count_bp">
                            ...
                            <xsl:choose>
                            <xsl:when test="$ALIGNED_READS &gt;= $MUT_DEPTH">
                                <td class="norotate" align="center" style="background: hsl(0, 100%, {$CELL_COLOR}%)">-</td>
                            </xsl:when>
                            <xsl:otherwise>
                                <td class="norotate" align="center">
                                    <xsl:choose><xsl:when test="$count_bp=5 or $count_bm=2">+</xsl:when>
			...
			<xsl:otherwise>
                            <xsl:choose>
                            <xsl:when test="sum(.//amplicon/@reads) &gt;= $MUT_DEPTH">
                                <td class="norotate" align="center" style="background: hsl(0, 100%, {$CELL_COLOR}%)">
                                    <xsl:choose>
                                    <xsl:when test="sum(.//amplicon/@reads) &gt;= $DEPTH_FILTER and sum(.//amplicon/@reads) &gt;= $MUT_DEPTH">+</xsl:when>
                                    <xsl:otherwise>-</xsl:otherwise>
                                    </xsl:choose>
			        </td>
                            </xsl:when>
			...
			<!--#This colors SNP assays by number of reads containing SNP, rather than number of reads aligning
			<xsl:choose>
			<xsl:when test="@type = 'SNP'">
                            <xsl:choose><xsl:when test="amplicon//snp[@name != 'unknown']/snp_call/@count &gt;= $MUT_DEPTH">
                            <xsl:variable name="CELL_COLOR">
                                <xsl:choose><xsl:when test="amplicon//snp[@name != 'unknown']/snp_call/@count div $TOTAL_READS &gt;= 0.05">30.0</xsl:when>
                                <xsl:otherwise><xsl:value-of select="(amplicon//snp[@name != 'unknown']/snp_call/@count div $TOTAL_READS) * -1400 + 100"/></xsl:otherwise></xsl:choose>
                            </xsl:variable>
                            <td class="norotate" align="center" style="background: hsl(0, 100%, {$CELL_COLOR}%)">
                            <xsl:choose>
                            <xsl:when test="amplicon/@reads &gt;= $DEPTH_FILTER and amplicon//snp[@name != 'unknown']/snp_call/@count &gt;= $MUT_DEPTH and amplicon//snp[@name != 'unknown']/snp_call/@percent &gt;= $PROP_FILTER">+</xsl:when>
                            <xsl:otherwise>-</xsl:otherwise>
                            </xsl:choose>
                            </td>
                            </xsl:when><xsl:otherwise><td class="norotate" align="center">-</td></xsl:otherwise></xsl:choose>
			</xsl:when>
			<xsl:otherwise>
                            <xsl:choose><xsl:when test="sum(.//amplicon/@reads) &gt;= $MUT_DEPTH">
                            <xsl:variable name="CELL_COLOR">
                                <xsl:choose><xsl:when test="sum(.//amplicon/@reads) div $TOTAL_READS &gt;= 0.05">30.0</xsl:when>
                                <xsl:otherwise><xsl:value-of select="(sum(.//amplicon/@reads) div $TOTAL_READS) * -1400 + 100"/></xsl:otherwise></xsl:choose>
                            </xsl:variable>
